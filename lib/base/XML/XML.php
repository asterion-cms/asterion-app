...
class XML
{
    ...
    {
        $title = (isset($options['title'])) ? $options['title'] : Parameter::code('meta_title_page');
        $link = (isset($options['link'])) ? $options['link'] : ASTERION_LOCAL_URL;
        $description = (isset($options['description'])) ? $options['description'] : Parameter::code('meta_description');
        return '<?xml version="1.0" encoding="ISO-8859-1"?>
                    ...
                    <channel>
                        ...
                        <administrator>' . Parameter::code('meta_title_page') . '</administrator>
                        <image>
                            <title>' . Parameter::code('meta_title_page') . '</title>
                            <url>' . ASTERION_LOGO . '</url>
                            <link>' . ASTERION_LOCAL_URL . '</link>
                            <description>' . Parameter::code('meta_description') . '</description>
                        </image>
                        ' . $content . '
                    </channel>
                    </rss>';
